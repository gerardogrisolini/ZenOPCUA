...
public struct DataValue: Promisable, OPCUAEncodable, Sendable {
    ...
    public let encodingMask: UInt8
    public let variant: Variant
    public let statusCode: StatusCodes
    public let sourceTimestamp: Date
    public let serverTimestamp: Date

    private static func parse(bytes: [UInt8], index: Int) -> ParsedValue {
        var index = index
        let encodingMask = bytes[index]
        index += 1

        var variant = Variant(type: DataType.null.rawValue)
        var statusCode = StatusCodes.UA_STATUSCODE_GOOD
    ...
    private init(parsed: ParsedValue) {
        self.encodingMask = parsed.encodingMask
        self.variant = parsed.variant
        self.statusCode = parsed.statusCode
        self.sourceTimestamp = parsed.sourceTimestamp
        self.serverTimestamp = parsed.serverTimestamp
    }

    public init(bytes: [UInt8], index: inout Int) {
        let parsed = Self.parse(bytes: bytes, index: index)
    ...
    public init(variant: Variant) {
        ...
        self.statusCode = .UA_STATUSCODE_GOOD
