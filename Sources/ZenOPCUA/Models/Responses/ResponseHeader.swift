...
struct ResponseHeader: OPCUADecodable, Sendable {
    var timestamp: Date
    ...
    init(bytes: [UInt8]) {
        guard bytes.count >= 24 else {
            ...
            requestHandle = 0
            serviceResult = .UA_STATUSCODE_BADDATAENCODINGINVALID
            serviceDiagnistics = 0
            stringTable = 0
            additionalHeader = AdditionalHeader(bytes: [0, 0, 0])
            return
        }
