...
enum RSACrypto {
    ...
    struct SecurityKeys: Sendable {
        let clientKeys: SecretKeys
        let serverKeys: SecretKeys
    }
    
    struct SecretKeys: Sendable {
    ...
    private struct EncryptionPrivateKeyBox: @unchecked Sendable {
    ...
    struct KeyPair: Sendable {
        ...
        var publicKey: _RSA.Encryption.PublicKey? {
            ...
            set { publicKeyBox.value = newValue }
        }
    ...
    private struct LockedBox<T: Sendable>: Sendable {
        let box: NIOLockedValueBox<T>
    }

    final class RuntimeContext: Sendable {
