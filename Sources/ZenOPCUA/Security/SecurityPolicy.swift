...
private actor SecurityPolicyConnectionSettingsRuntime {
    private struct SettingsState: Sendable {
        var messageSecurityMode: MessageSecurityMode = .none
        var includeServerThumbprintInOpn = false
    }

    private struct SettingsCache: Sendable {
        private let box = NIOLockedValueBox(SettingsState())
        ...
        func update(messageSecurityMode: MessageSecurityMode, includeServerThumbprintInOpn: Bool) {
            box.withLockedValue { state in
    ...
    nonisolated private let cache = SettingsCache()
    ...
    nonisolated func update(messageSecurityMode: MessageSecurityMode, includeServerThumbprintInOpn: Bool) {
        cache.update(messageSecurityMode: messageSecurityMode, includeServerThumbprintInOpn: includeServerThumbprintInOpn)
    }
}

private actor SecurityPolicyCryptoContextRuntime {
    private struct CryptoContextCache: Sendable {
        private let box = NIOLockedValueBox<RSACrypto.RuntimeContext?>(nil)

        var currentCryptoContext: RSACrypto.RuntimeContext? {
            box.withLockedValue { value in
                value
            }
        }

        func update(_ cryptoContext: RSACrypto.RuntimeContext?) {
            box.withLockedValue { value in
                value = cryptoContext
            }
        }
    ...
    nonisolated private let cache = CryptoContextCache()

    nonisolated var currentCryptoContext: RSACrypto.RuntimeContext? {
        cache.currentCryptoContext
    }

    nonisolated func update(_ cryptoContext: RSACrypto.RuntimeContext?) {
        cache.update(cryptoContext)
    }
}

private actor SecurityPolicyMaterialRuntime {
    private struct MaterialState: Sendable {
        var clientNonce: [UInt8] = []
        var localPrivateKey = Data()
        var localCertificate = Data()
        var localCertificateThumbprint = Data()
        var remoteCertificate = Data()
        var remoteCertificateThumbprint = Data()
    }

    private struct MaterialCache: Sendable {
        private let box = NIOLockedValueBox(MaterialState())

        var currentClientNonce: [UInt8] {
            box.withLockedValue { state in
                state.clientNonce
            }
        }

        var currentLocalPrivateKey: Data {
            box.withLockedValue { state in
                state.localPrivateKey
            }
        }

        var currentLocalCertificate: Data {
            box.withLockedValue { state in
                state.localCertificate
            }
        }

        var currentLocalCertificateThumbprint: Data {
            box.withLockedValue { state in
                state.localCertificateThumbprint
            }
        }

        var currentRemoteCertificate: Data {
            box.withLockedValue { state in
                state.remoteCertificate
            }
        }

        var currentRemoteCertificateThumbprint: Data {
            box.withLockedValue { state in
                state.remoteCertificateThumbprint
            }
        }

        func updateClientNonce(_ clientNonce: [UInt8]) {
            box.withLockedValue { state in
                state.clientNonce = clientNonce
            }
        }

        func updateLocalPrivateKey(_ localPrivateKey: Data) {
            box.withLockedValue { state in
                state.localPrivateKey = localPrivateKey
            }
        }

        func updateLocalCertificate(_ localCertificate: Data) {
            box.withLockedValue { state in
                state.localCertificate = localCertificate
            }
        }

        func updateLocalCertificateThumbprint(_ localCertificateThumbprint: Data) {
            box.withLockedValue { state in
                state.localCertificateThumbprint = localCertificateThumbprint
            }
        }

        func updateRemoteCertificate(_ remoteCertificate: Data) {
            box.withLockedValue { state in
                state.remoteCertificate = remoteCertificate
            }
        }

        func updateRemoteCertificateThumbprint(_ remoteCertificateThumbprint: Data) {
            box.withLockedValue { state in
                state.remoteCertificateThumbprint = remoteCertificateThumbprint
            }
        }
    }

    nonisolated private let cache = MaterialCache()

    nonisolated var currentClientNonce: [UInt8] {
        cache.currentClientNonce
    }

    nonisolated var currentLocalPrivateKey: Data {
        cache.currentLocalPrivateKey
    }

    nonisolated var currentLocalCertificate: Data {
        cache.currentLocalCertificate
    }

    nonisolated var currentLocalCertificateThumbprint: Data {
        cache.currentLocalCertificateThumbprint
    }

    nonisolated var currentRemoteCertificate: Data {
        cache.currentRemoteCertificate
    }

    nonisolated var currentRemoteCertificateThumbprint: Data {
        cache.currentRemoteCertificateThumbprint
    }

    nonisolated func updateClientNonce(_ clientNonce: [UInt8]) {
        cache.updateClientNonce(clientNonce)
    }

    nonisolated func updateLocalPrivateKey(_ localPrivateKey: Data) {
        cache.updateLocalPrivateKey(localPrivateKey)
    }

    nonisolated func updateLocalCertificate(_ localCertificate: Data) {
        cache.updateLocalCertificate(localCertificate)
    }

    nonisolated func updateLocalCertificateThumbprint(_ localCertificateThumbprint: Data) {
        cache.updateLocalCertificateThumbprint(localCertificateThumbprint)
    }

    nonisolated func updateRemoteCertificate(_ remoteCertificate: Data) {
        cache.updateRemoteCertificate(remoteCertificate)
    }

    nonisolated func updateRemoteCertificateThumbprint(_ remoteCertificateThumbprint: Data) {
        cache.updateRemoteCertificateThumbprint(remoteCertificateThumbprint)
    }
}

// Concurrency: mutable state is funneled through dedicated runtimes.
final class SecurityPolicy: Sendable {

    private let connectionSettingsRuntime = SecurityPolicyConnectionSettingsRuntime()
    private let cryptoContextRuntime = SecurityPolicyCryptoContextRuntime()
    private let materialRuntime = SecurityPolicyMaterialRuntime()
    private let localCryptoContext = RSACrypto.RuntimeContext()
    var clientNonce: [UInt8] {
        get { materialRuntime.currentClientNonce }
        set { materialRuntime.updateClientNonce(newValue) }
    }
    var localPrivateKey: Data {
        get { materialRuntime.currentLocalPrivateKey }
        set { materialRuntime.updateLocalPrivateKey(newValue) }
    }
    var localCertificate: Data {
        get { materialRuntime.currentLocalCertificate }
        set { materialRuntime.updateLocalCertificate(newValue) }
    }
    var localCertificateThumbprint: Data {
        get { materialRuntime.currentLocalCertificateThumbprint }
        set { materialRuntime.updateLocalCertificateThumbprint(newValue) }
    }
    var remoteCertificate: Data {
        get { materialRuntime.currentRemoteCertificate }
        set { materialRuntime.updateRemoteCertificate(newValue) }
    }
    var remoteCertificateThumbprint: Data {
        get { materialRuntime.currentRemoteCertificateThumbprint }
        set { materialRuntime.updateRemoteCertificateThumbprint(newValue) }
    }
    ...
    func updateConnectionSettings(messageSecurityMode: MessageSecurityMode, includeServerThumbprintInOpn: Bool) {
        connectionSettingsRuntime.update(
            messageSecurityMode: messageSecurityMode,
            includeServerThumbprintInOpn: includeServerThumbprintInOpn
        )
    }

    func updateCryptoContext(_ cryptoContext: RSACrypto.RuntimeContext?) {
        cryptoContextRuntime.update(cryptoContext)
    }
    ...
    var securityHeaderSize: Int {
        ...
        if remoteCertificate.count == 0 {
            ...
            let isSignAndEncrypt = connectionSettingsRuntime.currentMessageSecurityMode == .signAndEncrypt
            let includeThumbprintOverride = connectionSettingsRuntime.currentIncludeServerThumbprintInOpn
    ...
    var isAsymmetricSigningEnabled: Bool {
        return connectionSettingsRuntime.currentMessageSecurityMode != .none && localCertificate.count > 0
    }
    
    var isSymmetricSigningEnabled: Bool {
        let messageSecurityMode = connectionSettingsRuntime.currentMessageSecurityMode
        return localCertificate.count > 0 && messageSecurityMode != .none
            && (messageSecurityMode == .sign || messageSecurityMode == .signAndEncrypt)
    }
    
    var isAsymmetricEncryptionEnabled: Bool {
        return connectionSettingsRuntime.currentMessageSecurityMode != .none
   ...
   var isSymmetricEncryptionEnabled: Bool {
        let messageSecurityMode = connectionSettingsRuntime.currentMessageSecurityMode
        return remoteCertificate.count > 0 && messageSecurityMode != .none
            && messageSecurityMode == .signAndEncrypt
    }

    var isSigningEnabled: Bool { connectionSettingsRuntime.currentMessageSecurityMode != .none && localCertificate.count > 0 }
    var isEncryptionEnabled: Bool { 
        connectionSettingsRuntime.currentMessageSecurityMode == .signAndEncrypt 
            && localCertificate.count > 0 
    ...
    private var runtimeCryptoContext: RSACrypto.RuntimeContext {
        cryptoContextRuntime.currentCryptoContext ?? localCryptoContext
    }
