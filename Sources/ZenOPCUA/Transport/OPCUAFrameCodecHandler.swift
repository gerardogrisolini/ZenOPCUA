...
final class OPCUAFrameCodecHandler: ChannelDuplexHandler, Sendable {
    ...
    init(state: OPCUAConnectionState, snapshotProvider: ConnectionCoordinatorSnapshotProvider) {
        self.decoder = OPCUAFrameDecoder(state: state)
        self.encoder = OPCUAFrameEncoder(
            state: state,
            snapshotProvider: { snapshotProvider.currentSnapshot }
        )
    }
