...
private struct DecoderBufferData: Sendable {
    var parts: ByteBuffer?
    var pendingBuffer: ByteBuffer?
}

private struct DecoderBufferState: Sendable {
    private let box = NIOLockedValueBox(DecoderBufferData())
    ...
    func appendInboundBuffer(_ buffer: inout ByteBuffer) {
        box.withLockedValue { state in
            if state.pendingBuffer == nil {
                ...
                state.pendingBuffer!.writeBuffer(&buffer)
            }
        }
    }

    func takePendingBuffer() -> ByteBuffer? {
        box.withLockedValue { state in
    ...
    func storePendingBuffer(_ buffer: ByteBuffer?) {
        box.withLockedValue { state in
            state.pendingBuffer = buffer
        }
    }

    func takeParts() -> ByteBuffer? {
        box.withLockedValue { state in
            let buffer = state.parts
            state.parts = nil
            return buffer
        }
    }

    func storeParts(_ buffer: ByteBuffer?) {
        box.withLockedValue { state in
            state.parts = buffer
        }
    }
...
final class OPCUAFrameDecoder: Sendable {
    public typealias InboundOut = OPCUAFrame
    let byteBufferAllocator = ByteBufferAllocator()
    let state: OPCUAConnectionState
    private let bufferState = DecoderBufferState()
    ...
    func appendInboundBuffer(_ buffer: inout ByteBuffer) {
        bufferState.appendInboundBuffer(&buffer)
    }

    func decodeNextFrame() throws -> OPCUAFrame? {
        guard var buffer = bufferState.takePendingBuffer() else { return nil }
        if let frame = try decode(buffer: &buffer) {
            bufferState.storePendingBuffer(buffer.readableBytes > 0 ? buffer : nil)
            return frame
        }
        bufferState.storePendingBuffer(buffer)
        return nil
    }
    ...
    private func decode(buffer: inout ByteBuffer) throws -> OPCUAFrame? {
        ...
        if let chunkTypeRaw = buffer.getString(at: buffer.readerIndex + 3, length: 1),
           ...
           chunkType == .part {
            ...
            var partialFrame = bufferState.takeParts()
            if partialFrame == nil {
                partialFrame = byteBufferAllocator.buffer(capacity: count * lenght)
                guard let headerBytes = buffer.getBytes(at: buffer.readerIndex, length: 24) else {
                    return nil
                }
                partialFrame!.writeBytes(headerBytes)
            }
            ...
            for _ in 0..<count {
                ...
                partialFrame!.writeBytes(Array(b[24...]))
                buffer.moveReaderIndex(forwardBy: lenght)
            }

            bufferState.storeParts(partialFrame)
        ...
        if var f = bufferState.takeParts() {
    ...
    public func parse(buffer: inout ByteBuffer) throws -> OPCUAFrame? {
        ...
        let shouldDecrypt = (state.isSecurityEncryptionEnabled || signCompatibilityDecrypt)
            && type != .hello
            && type != .acknowledge
            && type != .error
        ...
        let shouldRemoveSignature = state.isSecuritySigningEnabled 
    ...
    private func decryptChunk(chunkBuffer: inout ByteBuffer) throws -> ByteBuffer {
        ...
        let isEncryptionEnabled = state.isSecurityEncryptionEnabled || isSignCompatibilityEncrypted
        let isAsymmetric = state.isSecurityAsymmetric

        let cipherTextBlockSize = isAsymmetric 
            ? state.currentAsymmetricCipherTextBlockSize
            : state.symmetricBlockSize
        ...
        do {
            if state.isSecurityAsymmetric {
                ...
                for _ in 0..<blockCount {
                    ...
                    let bytes = try state.decryptAsymmetric(dataToDencrypt)
                ...
                let bytes = try state.decryptSymmetric(dataToDencrypt)
                chunkBuffer.moveReaderIndex(to: 0)
                chunkBuffer.moveWriterIndex(to: header)
    ...
    public func verifyChunk(chunkBuffer: inout ByteBuffer) throws {
        ...
        if !state.verifySignature(signature, data: data) {
    ...
    var securityHeaderSize: Int {
        return state.isSecurityAsymmetric
            ? state.currentSecurityRemoteHeaderSize
            : 0
    }
    
    var signatureSize: Int {
        return state.isSecurityAsymmetric
            ? state.currentRemoteAsymmetricSignatureSize
            : state.currentSymmetricSignatureSize
    }
}
