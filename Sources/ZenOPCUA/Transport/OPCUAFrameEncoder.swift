...
final class OPCUAFrameEncoder: Sendable {
    let byteBufferAllocator = ByteBufferAllocator()
    let state: OPCUAConnectionState
    private let snapshotProvider: @Sendable () -> ConnectionCoordinatorSnapshot
    
    init(
        state: OPCUAConnectionState,
        snapshotProvider: @escaping @Sendable () -> ConnectionCoordinatorSnapshot
    ) {
        self.state = state
    ...
    func encode(frame: OPCUAFrame, out: inout ByteBuffer) throws {
        ...
        // Instead, we use transport bootstrap state to determine security state.
        
        var isEncryptionEnabled: Bool
        let isSigningEnabled: Bool
        let isAsymmetric: Bool
        let calculatedSignatureSize: Int
        let connectionSnapshot = snapshotProvider()
        
        if connectionSnapshot.shouldUseUnsignedBootstrapSecurityOnCurrentTransport {
            // Bootstrap transport - no security until the secure channel is active.
            ...
            if isAsymmetric {
                calculatedSignatureSize = state.currentAsymmetricSignatureSize
            } else {
                calculatedSignatureSize = state.currentSymmetricSignatureSize
            }
        }
        ...
        if messageTypeStr == "OPN" {
            ...
            if calculatedSize > 0 {
                ...
                let derivedSize = max(0, state.currentSecurityHeaderSize)
        ...
        while messageBuffer.readableBytes > 0 {
            ...
            if isSigningEnabled && calculatedSignatureSize > 0 {
                ...
                if isEncryptionEnabled {
                    if messageTypeStr == "OPN",
                       state.policyKind == .aes256Sha256RsaPss {
                ...
                let signature = try state.sign(dataToSign)
            ...
            if (isEncryptionEnabled) {
                ...
                if isAsymmetric {
                    ...
                    for _ in 0..<blockCount {
                        ...
                        dataEncrypted = try state.encryptAsymmetric(dataToEncrypt)
                    ...
                    let dataEncrypted = try state.encryptSymmetric(dataToEncrypt)
    ...
    var securityHeaderSize: Int { 
        max(0, state.currentSecurityHeaderSize)
    }

    var cipherTextBlockSize: Int { 
        if state.hasSymmetricKeys {
            return max(1, state.symmetricBlockSize)
        } else {
            return max(1, state.currentAsymmetricCipherTextBlockSize)
        }
    }
    ...
    var plainTextBlockSize: Int { 
        if state.hasSymmetricKeys {
            ...
            return max(1, state.currentAsymmetricPlainTextBlockSize)
        }
    }
}
